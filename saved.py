N_CORES = min(10, os.cpu_count() or 1)
...
def get_random_params(shape_name):
    """Generate random shape-specific parameters for geometry variation."""
    p = {}
    if shape_name == "valve":
...
# ==========================================
# Simulation Runner (Parallel Worker)
# ==========================================

def run_case(p):
    """Execute a single CFD simulation and extract data (runs in parallel)."""
    ...
    try:
        ...
        # Note: np.save only saves a single array. If you need to save multiple items (like shape_name and params),
        # you would typically save them as a dictionary within the .npy file, or revert to .npz for multiple arrays.
        return None
